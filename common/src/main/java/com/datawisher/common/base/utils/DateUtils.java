...
import com.datawisher.common.base.utils.validate.ValidateUtils;
...
//	private static Logger logger = Logger.getLogger(DateUtils.class);
	 ...
	 * 要用到的DATE Format的定义
	 */
	// Review SimpleDateFormat 非线程安全，使用 FastDateFormat
	...
	public static FastDateFormat sdfDateTime = FastDateFormat.getInstance(DateUtils.DATE_FORMAT_DATETIME);
	// Global FastDateFormat object
	public static FastDateFormat sdfDateOnly = FastDateFormat.getInstance(DateUtils.DATE_FORMAT_DATEONLY);
	...
	public static String toString(Date dt, String sFmt) {
		...
		return toString(dt, FastDateFormat.getInstance(sFmt));
	}
	 ...
	 * @history
	 */
	private static String toString(Date dt, FastDateFormat formatter) {
		String sRet = null;
...
//			logger.error(e);
	...
	public static String timestampToStringYMD(Timestamp timestamp) {
		FastDateFormat sdf = FastDateFormat.getInstance(DateUtils.DATE_FORMAT_DATEONLY);
	...
	public static boolean isBetween(Date date, Date start, Date end, int interModel, int compModel) {
		...
		switch (compModel) {
			...
			case COMP_MODEL_TIME: {
				format = FastDateFormat.getInstance("HHmmss");
				break;
			}
			case COMP_MODEL_DATETIME: {
				format = FastDateFormat.getInstance("yyyyMMddHHmmss");
				break;
			}
			default: {
...
//			logger.error(e);
	...
	public static Date getDateByStr(String dateStr) {
		...
		if (dateStr == null) {
			...
			formatter = FastDateFormat.getInstance("yyyy-MM-dd HH:mm");
...
//			logger.error(e);
	...
	public static long getDateDiff(String startDate, String endDate) {
		long diff = 0;
		try {
			Date date1 = FastDateFormat.getInstance("yyyy-MM-dd").parse(startDate);
	...
	public static String getStrFormTime(String form, Date date) {
		FastDateFormat sdf = FastDateFormat.getInstance(form);
	...
	public static Date dateFormate(String dateStr){
		Date value = null;
		try {
			value  = FastDateFormat.getInstance("yyyyMMddhhmmss").parse(dateStr);
		} catch (ParseException e) {
			e.printStackTrace();
		}
		return value;
	...
	public static void main(String[] args) {
		System.out.println(stringToDate("2017/05/01"));
